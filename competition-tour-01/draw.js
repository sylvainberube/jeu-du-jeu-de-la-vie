...
// Déplacement de la grille avec un cliquer-glisser
/*
function mousePressed() {
  grilleDeplacementX = mouseX;
  grilleDeplacementY = mouseY;
  if (simulateur.peutEtreDeplace) grilleDeplacementEnCours = true;
  if (modificationCellules === false && jeuCommence == true) {
    dragging = true;
  }

  if (mode == 4 && modeModification && modificationCellules) {
    if (planeur) {
      etampe(planeurRLE);
    } else {
...
function mouseDragged() {
  if (grilleDeplacementEnCours) {
    vue.modifierDecalage(mouseX-grilleDeplacementX, mouseY-grilleDeplacementY);
    grilleDeplacementX = mouseX;
    grilleDeplacementY = mouseY;
  }
  if (mode == 4 && modeModification && modificationCellules) {
...
function mouseReleased() {
  grilleDeplacementEnCours = false;
}
