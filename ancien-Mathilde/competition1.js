...
function debutDuJeu() {
    if (!jeuCommence) {
        accueil.show();
        texteNbJoueurs.html("Nombre de joueurs : " + str(sliderNbJoueurs.value()));
...
function tempsRestant() {
    encadreTempsRestant.style('visibility', 'visible');
    let tempsRest = int((tempsJeu - (millis() - modeModificationTemps))/100)/10;
    interieurTempsRestant.html('Il reste ' + str(tempsRest) + ' secondes.');
    tourActuel.html('Tour ' + str(toursJoues + 1));
}
